void fail_if_error(int ret) {
	if (ret != 0) {
		exit(-1);
	}
}

int init_sync_buffer(struct **rw_buf) {
	SyncBuffer *new_buf = malloc(sizeof(struct SyncBuffer));
	if (new_buf == NULL) {
		fatal_error("...");
	}

	new_buf->buffer = malloc(sizeof(BUF_MAX_LEN));
	if (new_buf->buffer == NULL) {
		fatal_error("...");
	}

	new_buf->buffer_len = 0;

	ret = pthread_mutex_init(&new_buf->mutex, NULL);
	if(ret != 0) {
		fatal_error("...");
	}

	
	*rw_buf = new_buf;
	return 0;
}

void pop_buffer(struct *SyncBuffer buf) {
	int ret = 0;

	if (buffer->buffer_len == 0) {
		return;
	}

	ret = pthread_mutex_lock(&(buf->mutex));
	
	buffer->buffer_len--;

	ret = pthread_mutex_unlock(&(buf->mutex));
}

void push_buffer(struct *SyncBuffer buf, Item *item) {
	if (buffer->buffer_len == BUF_MAX_LEN) {
		return;
	}

	while(1) {
		if (buffer->buffer_len == BUF_MAX_LEN) {
			continue;
		}

		capture_error(pthread_mutex_lock(&(buf->mutex)));
		
		buffer->buffer[buffer->buffer_len] = *item;
		buffer->buffer_len++;

		capture_error(pthread_mutex_unlock(&(buf->mutex)));
	}
}

void produce_item() {
	while(1) {
		if (buffer_len != BUF_MAX_LEN) {
			//wait 3-7 seconds before producing an item
			push_buffer()
		}
	}
}

void consume_item() {
	while(1) {
		if (buffer_len != 0) {
			//remove an item from the buffer
			pop_buffer()

			//decrement buffer length
			unlock_mutex(buffer_mutex);
			break;
		}
	}
}
